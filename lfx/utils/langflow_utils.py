...
def has_running_db_service():
    """Check if a real database service is available (not Noop)."""
    try:
        from lfx.services.database.service import NoopDatabaseService
        from lfx.services.deps import get_db_service

        db_service = get_db_service()
        return not isinstance(db_service, NoopDatabaseService)
    except Exception as e:
        logger.debug(f"Error checking for running db service: {e}")
        return False
